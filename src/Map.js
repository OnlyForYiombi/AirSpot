  ...
  useEffect(() => {
    const initMap = async () => {
      ...
      const adjustMaxIntensityByZoom = (zoomLevel) => {
        if (zoomLevel <= 5) return 90000; // 줌 레벨 5 이하: 최대 밀도
        if (zoomLevel <= 6) return 7000; // 줌 레벨 6 이하: 최대 밀도
        if (zoomLevel <= 7) return 6000; // 줌 레벨 7 이하: 최대 밀도
        if (zoomLevel <= 8) return 4000;  // 줌 레벨 8
        if (zoomLevel <= 9) return 2000;  // 줌 레벨 9
        if (zoomLevel <= 10) return 800;  // 줌 레벨 10
        if (zoomLevel <= 11) return 600;  // 줌 레벨 11
        if (zoomLevel <= 12) return 200; // 줌 레벨 12
        if (zoomLevel <= 13) return 180; // 줌 레벨 13
        if (zoomLevel <= 14) return 120;  // 줌 레벨 14
        if (zoomLevel <= 15) return 100;  // 줌 레벨 15
        return 70;                      // 줌 레벨 16, 17이상: 세부 표현
      };
      //기본 14
